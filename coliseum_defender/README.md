...
|---|---|---|
| Defender dataset | `adityajethani11/coliseum-defender-dataset` | Preprocessed + teacher-labeled training data |
| SFT checkpoint | `adityajethani11/coliseum-defender-sft` | Qwen2.5-1.5B after knowledge distillation SFT |
| GRPO checkpoint | `adityajethani11/coliseum-defender-grpo` | SFT model after in-environment GRPO training |
...
- [ ] After GRPO completes (~90 min): reload defender API with GRPO checkpoint
  ```bash
  curl -X POST http://localhost:8001/defender/reload?model_repo=adityajethani11/coliseum-defender-grpo
  ```
